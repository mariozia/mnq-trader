...
class DirectionalEngine:
    ...
    def run_cycle(self, now: datetime | None = None) -> dict:
        ...
        if position:
            return self._handle_in_position(prompt_ctx, snapshot, position)
    ...
    def _handle_in_position(
        ...
        if decision.action == Direction.CLOSE:
            trade = self.broker.close_at_price(
                snapshot.last_price, reason="LLM exit"
            )
            if trade:
    ...
    def _process_exit(self, trade: TradeRecord) -> dict:
        self.risk.record_trade(trade.pnl)
        self.state_store.record_trade(trade)
        self.state_store.state.position = None
        self.state_store.start_cooldown(self.config.engine.cooldown_minutes)
        logger.info(
            "Exit: %s P&L=$%.2f reason=%s",
    ...
    def _hard_close(self, reason: str) -> dict:
        position = self.broker.get_position()
        if not position:
